 ...
 *
 * Cache key: `<resourceHash>|<scope>|<name>|<kind>|<from>|<to>` — ranges that
 * differ by even a millisecond miss the cache (intentional: live mode passes
 * a fresh `now` every tick, and we want each tick's window to be honoured).
 *
...
export function useMetricSeriesCache(metrics: MetricsService) {
  ...
  function keyFor(ref: InstrumentRef, window: TimeWindow): string {
    return `${ref.resourceHash}|${ref.scopeName}|${ref.instrumentName}|${ref.kind}|${window.from}|${window.to}`
  }
   ...
   * hasn't elapsed yet.
   */
  function getPoints(ref: InstrumentRef, window: TimeWindow, bypassCache = false): Promise<MetricSeriesDto> {
    const key = keyFor(ref, window)
    const now = Date.now()
