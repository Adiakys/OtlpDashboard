...
/// <summary>
/// Surfaces the auth posture through <c>/healthz</c>: when the operator has
/// opted in to public access (<c>Dashboard:Auth:AllowAnonymous=true</c>) or
/// either token is unset (Development fallback), the check returns
...
internal sealed class AuthPostureHealthCheck : IHealthCheck
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            ["allowAnonymous"] = current.Auth.AllowAnonymous,
        ...
        if (current.Auth.AllowAnonymous)
        {
            return Task.FromResult(HealthCheckResult.Degraded(
                "Anonymous access explicitly enabled — every API is public.",
                data: data));
        }
